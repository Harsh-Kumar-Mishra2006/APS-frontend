...
const AddTeacherForm = ({ onSuccess, onCancel }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    // Validation
    const requiredFields = ['name', 'email', 'username', 'phone', 'teacherId'];
    ...
    try {
      ...
      setError(err.response?.data?.error || 'Failed to add teacher. Please try again.');
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="bg-white rounded-xl shadow-lg p-6">
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div className="col-span-2">
            <h3 className="text-lg font-semibold text-gray-700 mb-3 border-b pb-2 mt-2">Professional Information</h3>
          </div>
        ...
        <div className="flex gap-3 pt-4">
          <button
            type="submit"
            disabled={loading}
            className="flex-1 bg-green-600 text-white py-2 rounded-lg font-semibold hover:bg-green-700 transition duration-200 disabled:opacity-50 flex items-center justify-center gap-2"
          >
            {loading ? (
              <>
